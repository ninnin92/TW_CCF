...
%         k = fix((N-NOVERLAP)/(WINDOW-NOVERLAP)) : オーバーラップする数
%   columns.
%
...
% オプション関数の指定
maxlag = floor(nx/10); % 切り捨て
window = floor(nx/10); % 切り捨て
noverlap = 0;
...
Y = fliplr(buffer(y,maxlag+1,maxlag)'); % 配列を左右に反転
